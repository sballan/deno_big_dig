...
export class Sun {
  ...
  private initShaders(): void {
    ...
    if (!this.gl.getProgramParameter(this.shaderProgram, this.gl.LINK_STATUS)) {
      throw new Error(`Sun shader link failed: ${this.gl.getProgramInfoLog(this.shaderProgram)}`);
    }
  ...
  private compileShader(type: number, source: string): WebGLShader {
    ...
    if (!this.gl.getShaderParameter(shader, this.gl.COMPILE_STATUS)) {
      throw new Error(`Sun shader compile failed: ${this.gl.getShaderInfoLog(shader)}`);
    }
  ...
  render(projectionMatrix: Mat4, viewMatrix: Mat4, timeOfDay: number): void {
    ...
    const colorLoc = this.gl.getUniformLocation(this.shaderProgram, "uSunColor");
    ...
    // Keep depth testing enabled so terrain can occlude the sun
    // But clear depth buffer behind sun for sky effect
