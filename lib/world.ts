...
export class World {
    ...
    if (random < this.config.treeFrequency) {
      ...
      for (let y = 0; y < CHUNK_SIZE; y++) {
        ...
        if (worldY >= groundHeight && worldY < groundHeight + 6) {
          const localY = y;
          const trunkIndex = treeX + localY * CHUNK_SIZE +
            treeZ * CHUNK_SIZE * CHUNK_SIZE;
              ...
              for (let dz = -2; dz <= 2; dz++) {
                if (Math.abs(dx) + Math.abs(dz) <= 3) {
                  ...
                  if (
                    ...
                    const leafIndex = lx + localY * CHUNK_SIZE +
  ...
  getBlock(x: number, y: number, z: number): BlockType {
    ...
    const index = localX + localY * CHUNK_SIZE +
      localZ * CHUNK_SIZE * CHUNK_SIZE;
    ...
    for (let dx = -renderDistance; dx <= renderDistance; dx++) {
      for (let dy = -2; dy <= 2; dy++) {
        for (let dz = -renderDistance; dz <= renderDistance; dz++) {
          ...
          const cy = Math.max(
            0,
            Math.min(chunkY + dy, Math.floor(WORLD_HEIGHT / CHUNK_SIZE) - 1),
          );
          const cz = chunkZ + dz;
